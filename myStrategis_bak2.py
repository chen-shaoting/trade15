...
class kdj20macd60sma5_Strategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.macdCrossTwiceDuration=0
        self.macdAboveZeroDuration=0
        self.macdUnderZeroDuration=0
        self.kdjUnder40Duration=0
    ...
    def next(self):
        if self.macd[0] >0.0 and self.macd.signal[0]>0.0:
            self.macdAboveZeroDuration +=1
            self.macdUnderZeroDuration =0
        if self.macd[0] <-0.0 and self.macd.signal[0]< -0.0:
            self.macdUnderZeroDuration +=1
            self.macdAboveZeroDuration =0
        if self.k.percD[0]<40 and self.k.percDSlow[0]<40:
            self.kdjUnder40Duration +=1
        if self.k.percD[0] > 40 and self.k.percDSlow[0] > 40:
            self.kdjUnder40Duration =0

        if self.macdCrossDuration>=2:
            self.macdCrossTwiceDuration +=1
        ...
        if self.macd[-1] < self.macd.signal[-1] and self.macd[0]>self.macd.signal[0] and (self.macd[0]<0 and self.macd.signal[0]< 0):
            self.macdCross += 1
        #kdj K cross up 20 or kd golden cross when D is under 30
        if (self.k.percD[0]>20 and self.k.percD[-1]<20 ) or (self.k.percD[-1]<self.k.percDSlow[-1] and self.k.percD[0]>self.k.percDSlow[0] and self.k.percDSlow<30):
            self.kdjCross += 1
        #
        # if self.macdCrossTwiceDuration < 5 and
        # if (self.macdCross == 2 and self.kdjCross ==1 )or (self.kdjCross ==2 and self.macdCross==1):

        if not self.position:
            #判断市场的阶段：上升，下降，横盘，下跌后反转，上升后反转；寻找大幅回调点，反转点。

#MACD 持续在零轴下方，kdj 持续在40以下，而后出现金叉
            # if (self.macdUnderZeroDuration >45 and self.kdjUnder40Duration >15 and self.kdjCross>0 and self.kdjCrossDuration<15):#if (self.macdUnderZeroDuration >15 and self.kdjUnder40Duration >15 and self.kdjCross>1 and self.kdjCrossDuration<15):
            if (self.macdAboveZeroDuration >30 and self.kdjUnder40Duration>10 and self.kdjCross>0):
                self.order = self.buy(exectype=bt.Order.Market)
        else:

            c1 = self.macd[-1] > self.macd.signal[-1]
            c2 = self.macd[0] <= self.macd.signal[0]
            c3 = self.macd[0]>0.02 and self.macd.signal[0]>0.02 # macd 处在0线以上
            ...
            c7 = self.close[0] > self.buyprice *1.1 #
